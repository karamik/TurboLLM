...
Usage:
    python scripts/download_model.py --model neuralmagic/Meta-Llama-3-70B-Instruct-FP8 --output ./models/llama-70b-fp8  # noqa: E501
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Download and prepare models for TurboLLM inference engine."
    )
    parser.add_argument(
        "--model",
        type=str,
        required=True,
        help="Hugging Face model ID (e.g., neuralmagic/Meta-Llama-3-70B-Instruct-FP8 or meta-llama/Meta-Llama-3-70B-Instruct)",  # noqa: E501
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Hugging Face API token (optional, or set HUGGINGFACE_TOKEN env var for gated models).",  # noqa: E501
    )
    parser.add_argument(
        "--cache-dir",
        type=str,
...
def verify_model_repo(model_id: str, token: str = None):
    ...
    try:
        ...
        print(
            "   Please check the model name and ensure you have access (if it's a gated model like Llama)."  # noqa: E501
        )
        sys.exit(1)
...
def download_model(args):
    ...
    print(
        f"✅ Model found! Pipeline tag: {getattr(model_info, 'pipeline_tag', 'unknown')}"
    )
    ...
    try:
        ...
        print(
            f"   • Max Position Embeddings: {getattr(config, 'max_position_embeddings', 'N/A')}"  # noqa: E501
        )
    ...
    if is_fp8:
        ...
        print(
            "      vLLM will load these weights directly with maximum speed and low VRAM."  # noqa: E501
        )
    else:
    ...
    print(
        f"      python -m turbollm.serve --model {output_dir.resolve()} --quantization fp8 --kv-cache-dtype fp8 --port 8000"  # noqa: E501
    )
    print("=" * 50)
