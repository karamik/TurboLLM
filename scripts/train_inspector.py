#!/usr/bin/env python3
"""
train_inspector.py - Генерация синтетического датасета активаций и обучение ML-классификатора.  # noqa: E501
"""
...
NUM_SAMPLES = 5000
FEATURE_DIM = (
    12  # 4 статистики (mean, std, max, min) * 3 слоя (как в _extract_features)
)
NORMAL_MEAN = 0.2
NORMAL_STD = 0.1
...
def generate_synthetic_data(n_samples=1000, anomaly_ratio=0.2):
    ...
    X_normal = np.random.normal(
        loc=NORMAL_MEAN, scale=NORMAL_STD, size=(n_normal, FEATURE_DIM)
    )
    ...
    X_anomaly = np.random.normal(
        loc=NORMAL_MEAN + ANOMALY_SHIFT,
        scale=NORMAL_STD * 1.5,
        size=(n_anomaly, FEATURE_DIM),
    )
...
def train_and_save(
    model_path="inspector_model.pkl", scaler_path="inspector_scaler.pkl"
):
