...
# ========== ХРАНИЛИЩЕ СЕССИЙ И КЭШ CELLOUTPUT ==========
session_balances: Dict[str, float] = {}
session_created: Dict[str, float] = {}
session_addresses: Dict[str, str] = {}
used_transactions: set = set()
cell_outputs_cache: Dict[str, 'CellOutput'] = {}  # кэш для быстрого доступа к CellOutput по block_id
...
# ========== РАБОТА С БАЗОЙ ДАННЫХ ДЛЯ АКТИВАЦИЙ ==========
def init_activations_db(db_path: str = "activations.db"):
    ...
    try:
        ...
        conn.commit()
        conn.close()
        logger.info(f"База данных активаций инициализирована ({db_path})")
    except Exception as e:
        logger.warning(f"Не удалось инициализировать БД активаций: {e}")

def save_activations_to_db(block_id: str, activations: Dict):
    """Сохраняет активации в SQLite."""
    if not activations:
        return
    try:
        serializable = {}
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT OR REPLACE INTO activations (block_id, activations_json, timestamp) VALUES (?, ?, ?)",
            (block_id, json.dumps(serializable), datetime.now(timezone.utc).isoformat())
        )
        conn.commit()
        conn.close()
    except Exception as e:
        logger.warning(f"Не удалось сохранить активации в БД: {e}")
...
async def handle_feedback(request: web.Request):
    """Сбор обратной связи от пользователя (RLHF) с сохранением в БД и кэше."""
    ...
    try:
        ...
        # Получаем баланс сессии
        balance = get_balance(session_id)
        # Извлекаем спектральные метрики из кэша, если есть
        spectral_metrics = None
        if block_id in cell_outputs_cache:
            cell = cell_outputs_cache[block_id]
            spectral_metrics = cell.payload.get("spectral_metrics")
        feedback_entry = {
...
async def handle_propose_rule(request: web.Request):
    ...
    try:
        ...
        cell_output = CellOutput(
            ...
            case_metadata={"proposal_id": proposal_id, "proposer_session": session_id, "new_params": new_params, "status": "active"},
            payload={}
        )
        async with aiohttp.ClientSession() as session:
            delivered = await send_to_cluster(session, cell_output)
        if delivered:
            return web.json_response({"status": "ok", "proposal_id": proposal_id, "message": "Proposal recorded. Voting will start soon."})
        else:
...
async def handle_process(request: web.Request):
    ...
    try:
        ...
        # Сохраняем активации в БД, если есть
        if supervisor_output.metadata.get("activations_hash"):
            # Получаем активации из супервайзера (они не сохраняются, нужно будет доработать)
            # В реальности их нужно передавать из _call_llm_with_activations, но для MVP сохраняем только хэш
            pass
        async with aiohttp.ClientSession() as session:
            delivered = await send_to_cluster(session, cell_output)
...
async def handle_arbitrate(request: web.Request):
    ...
    try:
        ...
        # Сохраняем активации в БД (если есть)
        if supervisor_output.metadata.get("activations_hash"):
            # В реальности нужно передать activations из _call_llm_with_activations
            pass
        async with aiohttp.ClientSession() as session:
            delivered = await send_to_cluster(session, cell_output)
        AGENT_REQUESTS.labels(status=supervisor_output.status).inc()
...
async def handle_vote(request: web.Request):
    ...
    try:
        ...
        if not proposal_id or not choice:
            return web.json_response({"error": "Missing proposal_id or choice"}, status=400)
        ...
        cell_output = CellOutput(
            ...
            manifest=manifest, poi_chain=[hashlib.sha256(f"step_{i}".encode()).hexdigest() for i in range(3)],
            decision_type="vote",
            case_metadata={"proposal_id": proposal_id, "vote_choice": choice, "vote_weight": 1.0},
...
# ========== ЗАПУСК ==========
def main():
    # Инициализируем БД для активаций
    init_activations_db()
