...
и криптографическим доказательством инспекции (PoI).
"""
...
# Если ключ не задан, генерируем временный (не для продакшена)
...
class AdvancedGSpaceInspector(RealInspector):
    def __init__(self, model_path=None, scaler_path=None, auto_train=False):
        ...
        self.threshold_spectral = 0.85
        self.reference_vectors: Dict[str, Dict[int, torch.Tensor]] = {}
        self.adaptation_rate = 0.1  # коэффициент обновления эталона

    def set_reference_vector(self, prompt_type: str, activations: Dict[int, torch.Tensor]):
        """Сохраняет эталонное состояние скрытых слоев для чистого промпта."""
    ...
    def update_reference_vector(self, prompt_type: str, activations: Dict[int, torch.Tensor]):
        """Адаптивно обновляет эталон после успешной проверки (сглаживание)."""
        ...
        for layer, tensor in activations.items():
            if layer in ref:
                # Сглаживание: new_ref = (1 - α) * ref + α * current
    ...
    def analyze_advanced(self, prompt: str, preliminary_response: str, activations: Optional[Dict[int, torch.Tensor]] = None,
        ...
        if activations:
            ...
            # Переоценка аномалии
            if spec_metrics.get("total_layer_anomalies", 0) > 0 or drift > 0.35:
...
class AgentSupervisorCell:
    ...
    async def process(self, cell_input: CellInput, prompt_type: str = "default") -> CellOutputSupervisor:
        ...
        if not anomaly and confidence >= CONFIDENCE_THRESHOLD_HIGH:
            ...
            # Обновляем эталон после успешной проверки
            if activations:
    ...
    def _generate_proof(self, prompt: str, response: str, inspection_result: Dict) -> str:
        """Генерирует криптографическую подпись для доказательства инспекции."""
        payload = f"{prompt[:50]}:{response[:50]}:{inspection_result.get('confidence', 0)}:{inspection_result.get('anomaly_detected', False)}"
        signature = hashlib.sha256(payload.encode()).hexdigest()
        # В реальном продакшене использовать асимметричную подпись
        return signature
...
async def handle_arbitrate(request: web.Request):
    ...
    try:
        data = await request.json()
        case = ArbitrationCase(**data)
        prompt = f"""
        Вы — беспристрастный арбитражный судья. Рассмотрите следующий спор:
        Дело: {case.title}
        Описание: {case.description}
        Стороны: {', '.join(case.parties)}
        Аргументы сторон: {json.dumps(case.arguments, indent=2)}
        Доказательства: {', '.join(case.evidence)}
        ...
        cell_output = CellOutput(
            ...
            manifest=manifest, poi_chain=[hashlib.sha256(f"step_{i}".encode()).hexdigest() for i in range(3)],
            company_id=case.company_id, decision_type="arbitration",
            case_metadata={
                "case_id": case.case_id, "parties": case.parties,
            ...
            payload={"supervisor_status": supervisor_output.status, "cost": COST_ARBITRATION, "balance_after": new_balance}
        )
        async with aiohttp.ClientSession() as session:
            delivered = await send_to_cluster(session, cell_output)
        AGENT_REQUESTS.labels(status=supervisor_output.status).inc()
...
async def handle_vote(request: web.Request):
    ...
    try:
        data = await request.json()
        proposal = VoteProposal(**data)
        user_choice = data.get("choice")
        if user_choice not in proposal.options:
            return web.json_response({"error": "Invalid choice"}, status=400)
        now = datetime.now(timezone.utc)
        if now < proposal.start_date or now > proposal.end_date:
            return web.json_response({"error": "Voting is not active"}, status=400)
        ...
        cell_output = CellOutput(
            cell_id=CELL_ID, block_id=block_id, timestamp=datetime.now(timezone.utc).isoformat(),
            decision=f"Vote: {user_choice}",
            manifest=manifest, poi_chain=[hashlib.sha256(f"step_{i}".encode()).hexdigest() for i in range(3)],
            company_id=proposal.company_id, decision_type="vote",
            case_metadata={"proposal_id": proposal.proposal_id, "user_choice": user_choice, "vote_weight": 1.0},
            payload={"balance_after": new_balance, "cost": COST_VOTE, "proposal_title": proposal.title}
        )
        async with aiohttp.ClientSession() as session:
            delivered = await send_to_cluster(session, cell_output)
